...
public class PythonSession {
	 ...
	 */
	public static PythonSession acquireSession(Object requester) throws WekaException {
	...
	private PythonSession(String pythonCommand, boolean debug) throws IOException {
		...
		String tester = null;
		if (getClass().getClassLoader().getResource("pyCheck.py") != null) {
			try {
				java.nio.file.Files.copy(JniLoader.class.getClassLoader().getResourceAsStream("pyCheck.py"),
						java.nio.file.Paths
								.get(System.getProperty("java.io.tmpdir") + java.io.File.separator + "pyCheck.py"),
						java.nio.file.StandardCopyOption.REPLACE_EXISTING);
				tester = System.getProperty("java.io.tmpdir") + java.io.File.separator + "pyCheck.py";
			} catch (java.io.IOException e) {
				System.err.println("Could not copy file pyCheck.py to Temp extraction point.");
			}
		} else
			tester = "pyCheck.py";
	 ...
	 * @throws WekaException if python is not available
	 */
	private synchronized PythonSession getSession(Object requester) throws WekaException {
	...
	private void launchServer(boolean startPython) throws IOException {
		...
		if (startPython) {
			...
			if (getClass().getClassLoader().getResource("pyServer.py") != null) {
				try {
					java.nio.file.Files.copy(JniLoader.class.getClassLoader().getResourceAsStream("pyServer.py"),
							java.nio.file.Paths
									.get(System.getProperty("java.io.tmpdir") + java.io.File.separator + "pyServer.py"),
							java.nio.file.StandardCopyOption.REPLACE_EXISTING);
					serverScript = System.getProperty("java.io.tmpdir") + java.io.File.separator + "pyServer.py";
				} catch (java.io.IOException e) {
					System.err.println("Could not copy file pyServer.py to Temp extraction point.");
				}
			} else
				serverScript = "pyServer.py";
			ProcessBuilder processBuilder = new ProcessBuilder(m_pythonCommand, serverScript, "" + localPort,
					m_debug ? "debug" : "");
	 ...
	 */
	public PythonVariableType getPythonVariableType(String varName, boolean debug) throws WekaException {

		try {
			return ServerUtils.getPythonVariableType(varName, m_localSocket.getOutputStream(),
	 ...
	 * @throws WekaException if a problem occurs
	 */
	public void instancesToPython(Instances instances, String pythonFrameName, boolean debug) throws WekaException {
		try {
	 ...
	 * out numpy arrays of input and target features/columns. These arrays are named
	 * X and Y respectively in python. If there is no class set in the instances
	 * then only an X array is extracted.
	 *
	 * @param instances       the instances to transfer
	 ...
	 * @throws WekaException if a problem occurs
	 */
	public void instancesToPythonAsScikitLearn(Instances instances, String pythonFrameName, boolean debug)
			throws WekaException {
		try {
			ServerUtils.sendInstancesScikitLearn(instances, pythonFrameName, m_localSocket.getOutputStream(),
					m_localSocket.getInputStream(), m_log, debug);
		} catch (Exception ex) {
			throw new WekaException(ex);
		}
	 ...
	 *
	 * @param frameName the name of the pandas data frame to extract and convert to
	 *                  instances
	 * @param debug     true for debugging output
	 * @return an Instances object
	 * @throws WekaException if the named data frame does not exist in python or is
	 *                       not a pandas data frame
	 */
	public Instances getDataFrameAsInstances(String frameName, boolean debug) throws WekaException {
		try {
			return ServerUtils.receiveInstances(frameName, m_localSocket.getOutputStream(),
					m_localSocket.getInputStream(), m_log, debug);
	 ...
	 * @throws WekaException if a problem occurs
	 */
	public List<String> executeScript(String pyScript, boolean debug) throws WekaException {
		try {
			return ServerUtils.executeUserScript(pyScript, m_localSocket.getOutputStream(),
					m_localSocket.getInputStream(), m_log, debug);
		} catch (IOException ex) {
			throw new WekaException(ex);
		}
	 ...
	 * @throws WekaException if a problem occurs
	 */
	public boolean checkIfPythonVariableIsSet(String varName, boolean debug) throws WekaException {
		try {
	...
	/**
	 * Attempt to retrieve the value of a variable in python using serialization to
	 * Json. If successful, then the resulting Object is either a Map or List
	 ...
	 * @throws WekaException if a problem occurs
	 */
	public Object getVariableValueFromPythonAsJson(String varName, boolean debug) throws WekaException {
		try {
	 ...
	 * @throws WekaException if a problem occurs
	 */
	public String getVariableValueFromPythonAsPickledObject(String varName, boolean debug) throws WekaException {
		try {
			return ServerUtils.receivePickledVariableValue(varName, m_localSocket.getOutputStream(),
					m_localSocket.getInputStream(), false, m_log, debug);
		} catch (IOException ex) {
			throw new WekaException(ex);
		}
	}
	 ...
	 * executes a 'str(varName)' in python).
	 *
	 * @param varName the name of the variable to retrieve
	 * @param debug   true for debugging output
	 ...
	 * @throws WekaException if a problem occurs
	 */
	public String getVariableValueFromPythonAsPlainString(String varName, boolean debug) throws WekaException {
		try {
			return ServerUtils.receivePickledVariableValue(varName, m_localSocket.getOutputStream(),
					m_localSocket.getInputStream(), true, m_log, debug);
		} catch (IOException ex) {
			throw new WekaException(ex);
		}
	}
	...
	/**
	 * Get a list of variables that are set in python. Returns a list of two element
	 * string arrays. Each entry in the list is a variable. The first element of the
	 * array is the name of the variable and the second is its type in python.
	 *
	 * @param debug true if debugging info is to be output
	 * @return a list of variables set in python
	 * @throws WekaException if a problem occurs
	 */
	public List<String[]> getVariableListFromPython(boolean debug) throws WekaException {
		try {
			return ServerUtils.receiveVariableList(m_localSocket.getOutputStream(), m_localSocket.getInputStream(),
					m_log, debug);
		} catch (IOException ex) {
			throw new WekaException(ex);
		}
	...
	/**
	 * Push a pickled python variable value back into python. Deserializes the value
	 * in python.
	 *
	 ...
	 * @throws WekaException if a problem occurs
	 */
	public void setPythonPickledVariableValue(String varName, String varValue, boolean debug) throws WekaException {
		try {
			ServerUtils.sendPickledVariableValue(varName, varValue, m_localSocket.getOutputStream(),
					m_localSocket.getInputStream(), m_log, debug);
		} catch (IOException ex) {
			throw new WekaException(ex);
		}
	...
	public List<String> getPythonDebugBuffer(boolean debug) throws WekaException {
		try {
			return ServerUtils.receiveDebugBuffer(m_localSocket.getOutputStream(), m_localSocket.getInputStream(),
					m_log, debug);
		} catch (IOException ex) {
			throw new WekaException(ex);
		}
	...
	/**
	 * Retrieve an image from python. Assumes that the image in python is stored as
	 * a matplotlib.figure.Figure object. Returns a BufferedImage containing the
	 * image.
	 *
	 * @param varName the name of the variable in python that contains the image
	 * @param debug   true to output debugging info
	 * @return a BufferedImage
	 * @throws WekaException if the variable doesn't exist, doesn't contain a Figure
	 *                       object or there is a comms error.
	 */
	public BufferedImage getImageFromPython(String varName, boolean debug) throws WekaException {
		try {
			return ServerUtils.getPNGImageFromPython(varName, m_localSocket.getOutputStream(),
					m_localSocket.getInputStream(), m_log, debug);
		} catch (IOException ex) {
			throw new WekaException(ex);
		}
		...
		if (!m_shutdown) {
			try {
				...
				if (m_localSocket != null) {
					...
					if (m_debug) {
						List<String> outAndErr = ServerUtils.receiveDebugBuffer(m_localSocket.getOutputStream(),
	...
	/**
	 * Initialize the session. This needs to be called exactly once in order to run
	 * checks and launch the server. Creates a session singleton.
	 *
	 * @param pythonCommand the python command
	 * @param debug         true for debugging output
	 * @return true if the server launched successfully
	 * @throws WekaException if there was a problem - missing packages in python, or
	 *                       python could not be started for some reason
	 */
	public static boolean initSession(String pythonCommand, boolean debug) throws WekaException {
		if (s_sessionSingleton != null) {
	...
	public static void main(String[] args) {
		try {
			...
			String script = "from sklearn import datasets\nfrom pandas import DataFrame\ndiabetes = "
